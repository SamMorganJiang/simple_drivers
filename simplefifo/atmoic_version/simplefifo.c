...
static struct simplefifo_dev *sf;

static atomic_t sf_available = ATOMIC_INIT(1);

static int simplefifo_open(struct inode *inode, struct file *filp)
{
	if (!atomic_dec_and_test(&sf_available)) {
		atomic_inc(&sf_available);
		return -EINVAL;
	}
				...
				size_t count, loff_t *ppos)
{
	...
	if (copy_to_user(buf, dev->fifo + *ppos, count))
		ret = -EFAULT;
	else {
		*ppos += count;
		ret = count;
				...
				size_t count, loff_t *ppos)
{
	...
	if (copy_from_user(dev->fifo + *ppos, buf, count))
		ret = -EFAULT;
	else {
		*ppos += count;
		ret = count;
				...
				unsigned long arg)
{
	...
	switch (cmd) {
		case FIFO_CLEAR:
			memset(dev->fifo, 0, SIMPLEFIFO_SIZE);
